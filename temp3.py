        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # Additional penalty for domain mismatch with more nuanced approach
                    domain_mismatch_penalties = {
                        'machine_learning': {
                            'software_development': 0.4,  # 60% penalty - some transferable skills
                            'data_science': 0.2,          # 20% penalty - closely related
                            'backend_development': 0.5,   # 50% penalty - some overlap
                            'frontend_development': 0.6,  # 40% penalty - less overlap
                            'devops': 0.5                 # 50% penalty - some ML deployment overlap
                        },
                        'software_development': {
                            'machine_learning': 0.6,     # 40% penalty - different focus
                            'data_science': 0.5,         # 50% penalty - different skill set
                            'devops': 0.3,               # 30% penalty - deployment knowledge helps
                            'frontend_development': 0.2, # 20% penalty - closely related
                            'backend_development': 0.2   # 20% penalty - closely related
                        },
                        'data_science': {
                            'machine_learning': 0.2,     # 20% penalty - very related
                            'software_development': 0.4, # 60% penalty - different focus
                            'backend_development': 0.5,  # 50% penalty - some database overlap
                            'frontend_development': 0.6, # 40% penalty - less relevant
                            'devops': 0.6                # 40% penalty - different skill set
                        }
                    }
                    
                    # Detect candidate's primary domain based on their strongest evidence
                    ...
                    for domain_name, config in domain_evidence.items():
                        score = (
                            sum(1 for impl in config['core_implementations'] if impl in resume_text.lower()) * 3 +
                            sum(1 for impl in config['advanced_implementations'] if impl in resume_text.lower()) * 2 +
                            sum(1 for proof in config['deployment_proof'] if proof in resume_text.lower()) * 1
                        )
                        candidate_domain_scores[domain_name] = score
                    
                    # Find candidate's strongest domain
                    candidate_primary_domain = max(candidate_domain_scores.keys(), key=lambda k: candidate_domain_scores[k])
                    
                    # Apply domain mismatch penalty if significant mismatch
                    if (candidate_primary_domain != target_domain and 
                        candidate_domain_scores[candidate_primary_domain] > 3 and  # Candidate has significant experience in different domain
                        target_domain in domain_mismatch_penalties.get(candidate_primary_domain, {})):
                        
                        penalty_factor = domain_mismatch_penalties[candidate_primary_domain][target_domain]
                        final_score = int(final_score * (1 - penalty_factor))
                        
                        # Additional hard cap for significant domain mismatches
                        domain_mismatch_caps = {
                            ('machine_learning', 'software_development'): 50,
                            ('machine_learning', 'frontend_development'): 45,
                            ('machine_learning', 'backend_development'): 48,
                            ('software_development', 'machine_learning'): 45,
                            ('data_science', 'software_development'): 48,
                        }
                        
                        cap_key = (candidate_primary_domain, target_domain)
                        if cap_key in domain_mismatch_caps:
                            final_score = min(final_score, domain_mismatch_caps[cap_key])
